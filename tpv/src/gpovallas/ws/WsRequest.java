	...
	public <T> T execute(String accion, List<NameValuePair> params, Class<T> clase, int method, List<NameValuePair> headers, Gson gson) {
		...
		try {
			...
			/*if (response != null && ((WsResponse)response).error!=null && ((WsResponse)response).error.code==3000) {
				...
				LoginRequest autorizacion = new LoginRequest();
				Boolean loginResponse= autorizacion.execute(username, pass, ApplicationStatus.getInstance().getMac(GPOVallasApplication.context.getApplicationContext()), LoginResponse.class);				
			...
			try {
				...
				if (wsResponse != null)	email.enviarRequestError(wsResponse.getResponseString(), accion, clase.getName());
