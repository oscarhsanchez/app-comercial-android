...
public class Utils {
	 ...
	 *
	 * @param breadcrumb1
	 * TÃ­tulo (Typeface Bold)
	 * @param breadcrumb2
	 * Detalle (Typeface Normal)
	 * @param ctx
	 * Contexto de la AplicaciÃ³n
	 */
	public static void setBreadcrumbTitle(Activity ctx, String breadcrumb1, String breadcrumb2)
	{
		TextView breadcrumb_n1 = (TextView)ctx.findViewById(R.id.header_breadcrumb_n1);
		TextView breadcrumb_n2 = (TextView)ctx.findViewById(R.id.header_breadcrumb_n2);
	...
	public static String getTokenFromString(String pKey) throws NoSuchAlgorithmException, UnsupportedEncodingException {
		MessageDigest md = MessageDigest.getInstance("SHA-1");			
		return Base64.encode(Utils.byteArrayToHexString(md.digest(("psub/!*20aa" + pKey).getBytes("UTF-8"))));	
	}
	
	public static Integer generateCodCli(SQLiteDatabase db) {
		Cursor c =  db.rawQuery("SELECT *  FROM ENTITYUSER WHERE token = '"+GPOVallasApplication.usuarioAsignado.token+"'", null);
		if (c.moveToFirst()) {
			EntityUser user = (EntityUser) Database.getObjectByCursor(db, EntityUser.class, c);
			user.num_captacion = user.num_captacion +1;
			
			ContentValues reg = new ContentValues();
			reg.put("num_captacion", user.num_captacion);
			boolean result =  db.update("ENTITYUSER", reg, "token = '" + GPOVallasApplication.usuarioAsignado.token + "'", null) > 0;
			
			if(result){
				return user.num_captacion; 
			}else{
				return null;
			}
		} else
	...
	public static Integer generateCodInventario(SQLiteDatabase db) {
		Cursor c =  db.rawQuery("SELECT *  FROM ENTITYUSER WHERE token = '"+GPOVallasApplication.usuarioAsignado.token+"'", null);
		if (c.moveToFirst()) {
			EntityUser user = (EntityUser) Database.getObjectByCursor(db, EntityUser.class, c);
			user.num_inventario = user.num_inventario +1;
	
			ContentValues reg = new ContentValues();
			reg.put("num_inventario", user.num_inventario);
			boolean result =  db.update("ENTITYUSER", reg, "token = '" + GPOVallasApplication.usuarioAsignado.token + "'", null) > 0;
			if(result){
				return user.num_inventario; 
			}else{
				return null;
			}
		} else
	...
	public static Integer generateCodAlmacen(SQLiteDatabase db) {
		Cursor c =  db.rawQuery("SELECT *  FROM ENTITYUSER WHERE token = '"+GPOVallasApplication.usuarioAsignado.token+"'", null);
		if (c.moveToFirst()) {
			EntityUser user = (EntityUser) Database.getObjectByCursor(db, EntityUser.class, c);
			user.num_mov_almacen = user.num_mov_almacen +1;
			
			ContentValues reg = new ContentValues();
			reg.put("num_mov_almacen",user.num_mov_almacen);
			boolean result =  db.update("ENTITYUSER", reg, "token = '" + GPOVallasApplication.usuarioAsignado.token + "'", null) > 0;
			
			if(result){
				return user.num_mov_almacen; 
			}else{
				return null;
			}
		} else
	...
	public static Integer generateCodIncidencia(SQLiteDatabase db) {
		Cursor c =  db.rawQuery("SELECT *  FROM ENTITYUSER WHERE token = '"+GPOVallasApplication.usuarioAsignado.token+"'", null);
		if (c.moveToFirst()) {
			EntityUser user = (EntityUser) Database.getObjectByCursor(db, EntityUser.class, c);
			user.num_incidencia = user.num_incidencia +1;
			
			ContentValues reg = new ContentValues();
			reg.put("num_incidencia", user.num_incidencia);
			boolean result =  db.update("ENTITYUSER", reg, "token = '" + GPOVallasApplication.usuarioAsignado.token + "'", null) > 0;
			
			if(result){
				return user.num_incidencia; 
			}else{
				return null;
			}
		} else
	 ...
	 public static Bitmap DownloadFullFromUrl(String imageFullURL) {
		...
		public static String calculaFecha(String fecha) {
			...
			try {
				return Utils.diferenciaEntreFechas(formatter.parse(fecha).getTime(), System.currentTimeMillis());
			} catch (Exception e1) {
				return "Ahora mismo";
			}
		}
		 ...
		 public static int calculateInSampleSize(
		  ...
		  public static int getOrientation(Bitmap bitmap, String filePath){
			  try
			  {
			      ...
			      else if (orientation == 3)      return 180;
		  ...
		  public static Bitmap scaleImage(Context context, Uri photoUri) throws IOException {
		        ...
		        if (rotatedWidth > 100 || rotatedHeight > 100) {
		            float widthRatio = ((float) rotatedWidth) / ((float) 100); //100 = MAX_IMAGE_DIMENSION 
