...
import gpovallas.ws.sender.request.SendBriefsRequest;
...
public class Sender {
    ...
    public boolean sendBriefs() {
        Sender.setDatosEnviando(db, GPOVallasConstants.DB_TABLE_BRIEF);
        WsResponse response = (new SendBriefsRequest()).execute(WsResponse.class);
        if (response != null && !response.failed()) {
            Log.i("guarda brief", "OK");
            Sender.setDatosEnviadosOk(db, GPOVallasConstants.DB_TABLE_BRIEF);
            GPOVallasApplication.guardarLogPeticion(db, this.getClass().getName(), "sendBrief", "OK");
            return true;
        } else {
            Log.i("guarda brief", "FALSE");
            Sender.setDatosEnviadosKo(db, GPOVallasConstants.DB_TABLE_BRIEF);
            GPOVallasApplication.guardarLogPeticion(db, this.getClass().getName(), "sendBrief", "FAILED");
            return false;
        }
    }
